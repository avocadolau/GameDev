...
bool App::Awake()
{
	...
	if(ret == true)
	{
		title.create(config_node.child("app").child("title").child_value()); // L01: TODO 4: Read the title from the config file
		...
		while(item != NULL && ret == true)
		{
			...
			node = config_node.child(item->data->name.GetString());
...
bool App::LoadConfig()
{
	...
	if (result == NULL) LOG("Could not load map xml config.xml ERROR: %s", result.description());			// es como un print f
	else {
