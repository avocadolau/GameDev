	...
	if (frontier.Pop(curr))
	{
		...
		for (uint i = 0; i < 4; ++i)
		{
			if (IsWalkable(neighbors[i].x, neighbors[i].y))
			{
				if (visited.Find(neighbors[i]) == -1)
				{
					uint cost = MovementCost(neighbors[i].x, neighbors[i].y) + costSoFar[curr.x][curr.y];
					if (costSoFar[neighbors[i].x][neighbors[i].y]==0)
						costSoFar[neighbors[i].x][neighbors[i].y] = cost;
					else if (costSoFar[neighbors[i].x][neighbors[i].y]>cost)
						costSoFar[neighbors[i].x][neighbors[i].y] = cost;
					
					frontier.Push(neighbors[i], costSoFar[neighbors[i].x][neighbors[i].y]);
					visited.Add(neighbors[i]);
