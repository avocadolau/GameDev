...
bool Scene::Update(float dt)
{
	...
	if (app->input->GetKey(SDL_SCANCODE_1) == KEY_DOWN) app->map->PropagateAStar(1);

	if (app->input->GetKey(SDL_SCANCODE_1) == KEY_REPEAT &&
		app->input->GetKey(SDL_SCANCODE_LSHIFT) == KEY_REPEAT) app->map->PropagateAStar(1);

	if (app->input->GetKey(SDL_SCANCODE_2) == KEY_DOWN) app->map->PropagateAStar(2);

	if (app->input->GetKey(SDL_SCANCODE_2) == KEY_REPEAT &&
		app->input->GetKey(SDL_SCANCODE_LSHIFT) == KEY_REPEAT) app->map->PropagateAStar(2);

	if (app->input->GetKey(SDL_SCANCODE_3) == KEY_DOWN) app->map->PropagateAStar(3);

	if (app->input->GetKey(SDL_SCANCODE_3) == KEY_REPEAT &&
		app->input->GetKey(SDL_SCANCODE_LSHIFT) == KEY_REPEAT) app->map->PropagateAStar(3);
	...
	if (app->input->GetMouseButtonDown(3) == KEY_DOWN)
	{
		iPoint p;
		app->input->GetMousePosition(p.x, p.y);
		app->map->ComputePathAStar(p.x - app->render->camera.x, p.y - app->render->camera.y);
	}
