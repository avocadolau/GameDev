		...
		for (int x = 0; x < mapLayerItem->data->width; x++)
		{
			for (int y = 0; y < mapLayerItem->data->height; y++)
			{
				...
				if (gid > 0) {
					...
					tileset = GetTilesetFromTileId(gid);
...
bool Map::LoadProperties(pugi::xml_node& node, Properties& properties)
{
	...
	for (pugi::xml_node propertyNode = node.child("properties").child("property"); propertyNode!=NULL; propertyNode = propertyNode.next_sibling())
	{
		Properties::Property* p = new Properties::Property();
		p->name = propertyNode.attribute("name").as_string();
		p->value = propertyNode.attribute("value").as_int();
	}
